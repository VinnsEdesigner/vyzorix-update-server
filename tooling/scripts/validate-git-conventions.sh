...
if ! git rev-parse --git-dir > /dev/null 2>&1; then
    echo "⚠️  Not in a git repository - skipping convention check"
    exit 0
fi
...
if [[ -z "$COMMIT_MSG" ]]; then
    echo "⚠️  No commit message found - skipping"
    exit 0
fi
...
if [[ "$COMMIT_MSG" =~ $COMMIT_PATTERN ]]; then
    ...
    echo "   ⚠️  Commit doesn't follow conventional commits format"
...
if [[ "$COMMIT_MSG" == merge* ]] || [[ "$COMMIT_MSG" == Merge* ]]; then
    echo "   ℹ️  Merge commit detected - convention check skipped"
fi

# Check for revert commits
if [[ "$COMMIT_MSG" == revert* ]]; then
    echo "   ℹ️  Revert commit detected - convention check skipped"
fi
...
if [[ "$CURRENT_BRANCH" =~ $BRANCH_PATTERN ]]; then
    ...
    echo "   ⚠️  Branch doesn't follow common naming convention"
    echo "   Expected patterns: main, develop, feature/*, fix/*, hotfix/*, release/*"
fi

echo ""
echo "✅ Git convention check complete!"
