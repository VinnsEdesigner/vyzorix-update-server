...
export function SignedCommandExample() {
  ...
  const { get, post, hasCredentials, error, fetchCredentials } = useSignedApi({
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Demonstrates request signing for API calls</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        {/* Status indicator */}
        <div className="flex items-center gap-2 text-sm">
          <span
            className={`w-2 h-2 rounded-full ${hasCredentials ? "bg-green-500" : "bg-yellow-500"}`}
          />
        ...
        {/* Result display */}
        {commandResult && (
          <div className="p-3 bg-muted rounded-md font-mono text-sm">{commandResult}</div>
        )}
        ...
        <div className="text-xs text-muted-foreground space-y-1 pt-2 border-t">
          <p>
            <strong>How signing works:</strong>
          </p>
...
export function useSignedApiQuery() {
  ...
  return {
     ...
     */
    sendCommand: (deviceId: string, command: string, args?: Record<string, unknown>) => {
     ...
     */
    updateDevice: (deviceId: string, updates: Record<string, unknown>) => {
      return api.patch<{ success: boolean }>(`/v1/device/${encodeURIComponent(deviceId)}`, updates);
    },
    ...
    deleteDevice: (deviceId: string) => {
      return api.delete<{ success: boolean }>(`/v1/device/${encodeURIComponent(deviceId)}`);
    },
