...
find internal -maxdepth 1 -type d 2>/dev/null | while read dir; do
    if [ -d "$dir" ] && [ "$dir" != "internal" ]; then
        ...
        if [ ! -d "internal/infrastructure/$name" ] && [ ! -d "internal/domain/$name" ] && [ ! -d "internal/application/$name" ]; then
            ...
            echo "    $name/: $files files (NO migration exists)"
        fi
    fi
done
...
if [ -f "internal/api/server.go" ]; then
    echo "  Status:   API Server (needs NEW version)"
...
echo ""
echo "OLD structure (being phased):"
echo "    pkg/: config, crypto, logging, models, storage"
echo "    internal/: audit, email, fcm, ssr, command_signer"
echo "    internal/api/handlers/: flat handlers"
echo "    main.go: OLD entry point"
echo ""
