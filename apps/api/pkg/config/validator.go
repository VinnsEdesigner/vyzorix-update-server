...
// Validator validates configuration values including format and ranges.
...
func NewValidator(cfg Config) *Validator {
	return &Validator{config: cfg, errs: make([]ValidationError, 0)}
}
...
func (v *Validator) validateOrigins() {
...
func (v *Validator) validateSecuritySettings() {
	// ENFORCE_HMAC validation (already exists but ensure consistency)
	_ = v.config.EnforceHMAC
}

// validateTurnstile validates Turnstile configuration.
func (v *Validator) validateTurnstile() {
...
func (v *Validator) validateSigning() {
...
func (v *Validator) validateSession() {
	// SESSION_MAX_AGE: must be positive
...
func (v *Validator) validateRateLimits() {
	// Validate rate limit env vars if set
...
func (v *Validator) validateTimeWindows() {
	// JWT_DURATION_HOURS: must be positive
	if v.config.JWTDuration <= 0 {
		v.addError("JWT_DURATION_HOURS", "must be a positive duration")
	}

	// Validate expiry durations are reasonable
...
// ValidateConfig runs validation on the current config and returns an error if invalid.
