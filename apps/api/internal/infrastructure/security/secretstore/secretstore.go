...
// Store manages encrypted storage of per-device command secrets.
type Store struct {
...
// New creates a new SecretStore.
func New(baseDir string, masterKeyBase64 string) (*Store, error) {
...
func (s *Store) Set(deviceID string, secret string) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *Store) secretPath(deviceID string) string {
	return filepath.Join(s.baseDir, deviceID+".bin")
}

func (s *Store) getSecret(deviceID string) (string, error) {
	secretPath := s.secretPath(deviceID)
