...
func (r *EmailVerificationRepository) Delete(ctx context.Context, id string) error {
	_, err := r.db.ExecContext(ctx, `DELETE FROM email_verifications WHERE id = ?`, id)
	return err
}
...
func (r *EmailVerificationRepository) DeleteByOperator(ctx context.Context, operatorID string) error {
	_, err := r.db.ExecContext(ctx, `DELETE FROM email_verifications WHERE operator_id = ?`, operatorID)
	return err
}
