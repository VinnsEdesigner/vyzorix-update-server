...
// wsMessage represents a WebSocket message in the subscription protocol.
...
// Client represents a single subscription WebSocket client.
type Client struct {
	conn     *websocket.Conn
...
// NewSubscriptionHandler creates a subscription handler (alias for compatibility).
func NewSubscriptionHandler(
	...
	return NewHandler(hub, resolver, authMw, log)
}
...
func (c *Client) subscribeDeviceUpdates(id string, sub subscribePayload) {
	deviceID := getStringVar(sub.Variables, "deviceId")

	unsubscribe := c.handler.hub.SubscribeDeviceUpdates(c.operator.ID, deviceID, func(data interface{}) {
		c.sendMessage(wsMessage{
			Type:    "next",
			ID:      id,
			Payload: mustMarshal(map[string]interface{}{"deviceUpdated": data}),
		})
	})
	...
	c.sendMessage(wsMessage{
		Type:    "next",
		ID:      id,
		Payload: json.RawMessage(`{"data":{"deviceUpdated":null}}`),
	})
}
...
func (c *Client) subscribeTelemetry(id string, sub subscribePayload) {
	...
	unsubscribe := c.handler.hub.SubscribeTelemetry(c.operator.ID, deviceID, func(data interface{}) {
		c.sendMessage(wsMessage{
			Type:    "next",
			ID:      id,
			Payload: mustMarshal(map[string]interface{}{"telemetryReceived": data}),
		})
	})
	...
	c.sendMessage(wsMessage{
		Type:    "next",
		ID:      id,
		Payload: json.RawMessage(`{"data":{"telemetryReceived":null}}`),
	})
}
...
func (c *Client) subscribeCommandStatus(id string, sub subscribePayload) {
	dispatchID := getStringVar(sub.Variables, "dispatchId")

	unsubscribe := c.handler.hub.SubscribeCommandStatus(c.operator.ID, dispatchID, func(data interface{}) {
		c.sendMessage(wsMessage{
			Type:    "next",
			ID:      id,
			Payload: mustMarshal(map[string]interface{}{"commandStatusChanged": data}),
		})
	})
	...
	c.sendMessage(wsMessage{
		Type:    "next",
		ID:      id,
		Payload: json.RawMessage(`{"data":{"commandStatusChanged":null}}`),
	})
}
...
// getStringVar safely extracts a string variable from the variables map.
...
// mustMarshal marshals data to JSON, returns empty object on error.
