		...
		return filepath.Walk(dir, func(p string, info os.FileInfo, err error) error {
			...
			if strings.HasSuffix(p, ext) {
				if data, err := os.ReadFile(p); err == nil {
					sCollectGoAST(string(data), collect)
				}
			}
			return nil
		})
...
func sVerifyHandlers(_ *sSpec, _ *sImpl, root string) {
	...
	if _, err := os.Stat(handlerPath); err == nil {
		...
		fmt.Printf("     handlers/auth/auth_settings.go - NOT FOUND\n")
		atomic.AddUint64(&settingsFailCount, 1)
	}
}
...
func sVerifyDomain(_ *sSpec, _ *sImpl, root string) {
	...
	if _, err := os.Stat(domainPath); err != nil {
		fmt.Printf("     domain/operator/ - DIRECTORY NOT FOUND\n")
		atomic.AddUint64(&settingsFailCount, 1)
	} else {
		fmt.Printf("     domain/operator/\n")
		atomic.AddUint64(&settingsPassCount, 1)
	}
	files := []string{"operator_entity.go", "settings_types.go"}
	for _, f := range files {
		fp := filepath.Join(domainPath, f)
...
func sVerifyInfra(_ *sSpec, _ *sImpl, root string) {
	...
	if _, err := os.Stat(infraPath); err == nil {
		fmt.Printf("     infrastructure/storage/operator_storage.go\n")
		atomic.AddUint64(&settingsPassCount, 1)
	} else {
		fmt.Printf("     infrastructure/storage/operator_storage.go - NOT FOUND\n")
		atomic.AddUint64(&settingsFailCount, 1)
	}
}
...
func sVerifyApplication(_ *sSpec, _ *sImpl, root string) {
	...
	if _, err := os.Stat(appPath); err == nil {
		fmt.Printf("     application/auth/\n")
		atomic.AddUint64(&settingsPassCount, 1)
	} else {
		fmt.Printf("     application/auth/ - DIRECTORY NOT FOUND\n")
		atomic.AddUint64(&settingsFailCount, 1)
	}
}
...
func sVerifyRoutes(_ *sSpec, _ *sImpl, root string) {
	...
	if _, err := os.Stat(routePath); err == nil {
		fmt.Printf("     auth/auth_routes.go (contains settings routes)\n")
		atomic.AddUint64(&settingsPassCount, 1)
	} else {
		fmt.Printf("     auth/auth_routes.go - NOT FOUND\n")
		atomic.AddUint64(&settingsFailCount, 1)
	}
}
...
func sVerifySchema(_ *sSpec, _ *sImpl, root string) {
	...
	if _, err := os.Stat(entityPath); err == nil {
		fmt.Printf("     Schema defined in operator_entity.go\n")
		atomic.AddUint64(&settingsPassCount, 1)
	} else {
		fmt.Printf("     operator_entity.go NOT FOUND\n")
		atomic.AddUint64(&settingsFailCount, 1)
	}
}
